async function fetchRecipeData() {
  ...
  function displayRecipe(recipe) {
    ...
    imageEl.src = recipe.image || 'https://via.placeholder.com/400x300?text=No+Image';
    ...
    if (recipe.readyInMinutes) {
      document.getElementById('ready-time').textContent = `⏱ ${recipe.readyInMinutes} min`;
    }
    if (recipe.servings) {
      document.getElementById('servings').textContent = `🍽 ${recipe.servings} servings`;
    }
    if (recipe.healthScore) {
      document.getElementById('health-score').textContent = `❤️ ${recipe.healthScore} health`;
    }
    if (recipe.aggregateLikes) {
      document.getElementById('likes').textContent = `👍 ${recipe.aggregateLikes} likes`;
    }

    // Set diet tags
    const dietTagsEl = document.getElementById('diet-tags');
    if (recipe.diets && recipe.diets.length > 0) {
      dietTagsEl.innerHTML = recipe.diets.map(diet => 
    ...
    if (recipe.dishTypes && recipe.dishTypes.length > 0) {
      dishTypesEl.innerHTML = recipe.dishTypes.map(type => 
